...
const Dashboard = () => {
   ...
   const [currentlyDeletingFile, setCurrentlyDeltingFile] = useState<string | null>(
      null
   )
   ...
   const { mutate: deleteFile } = trpc.deleteFile.useMutation({
      ...
      onMutate: ({ id }) => {
         setCurrentlyDeltingFile(id)
      },
      onSettled: () => {
         setCurrentlyDeltingFile(null)
      }
   })
               ...
               <li key={file.id}
                  ...
                  <div className="px-6 mt-4 grid grid-cols-3 place-items-center py-2 gap-6 text-xs text-zinc-500">
                     <div className="flex items-center gap-2">
                        ...
                        {format(new Date(file.createdAt), "MMM yyyy dd")}
                     </div>
